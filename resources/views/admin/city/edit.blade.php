...
@section('title')
    <h1><i class="fa fa-list"></i>  Edit Data Kabupaten/Kota di Provinsi {{$province->name}}</h1>
@endsection
@section('breadcrumbs')
    {{ Breadcrumbs::render('Edit Data', $province, $city) }}
@endsection
@section('content')
<div class="col-md-12">
    <div class="tile">
      ...
      <div class="tile-body">
        <form action="{{ route('city.update', [$province, $city])}}" method="post" class="needs-validation" novalidate>
            @csrf
            @method('PUT')
            <div class="form-group">
                <label class="control-label">Nama Kabupaten/Kota</label>
                <input type="hidden" name="province" value="{{$province->id}}">
                <input class="form-control @error ('city') is-invalid @enderror" name="city" type="text" placeholder="Masukkan Kabupaten/Kota" value="{{$city->name}}">
                @error('city')
